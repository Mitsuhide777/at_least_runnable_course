...
//                properties.removeIf(f -> f.getValue("transient") != null);
/*                for (PropertyDescriptor p:
                        properties) {
                    if (p.getName().equals("class") || p.getValue("transient") != null)
                        properties.remove(p);
                }*/
/*                ListIterator<PropertyDescriptor> l = properties.listIterator();

                while (l.hasNext()) {
                    PropertyDescriptor p = l.next();
                    if (p.getName().equals("class") || p.getValue("transient") != null)
                        properties.remove(p);
                }*/
                LinkedList<PropertyDescriptor> transient_properties = new LinkedList<>();
                for (PropertyDescriptor p:
                        properties) {
                    if (p.getName().equals("class") || p.getValue("transient") != null)
                        transient_properties.add(p);
                }
                properties.removeAll(transient_properties);
                ...
                for (PropertyDescriptor property: properties) {
                    try {
                        ...
                        if (propertyName.equals("eventHookupInfo"))
                        {
                            System.out.print("");
                        }
...
//                                else if (!serialized.contains(obj) && (readMethod.getReturnType().isArray())) {
                                else if (!serialized.contains(obj) && propertyName.equals("eventHookupInfo")) {
                                    jsonWriter.writeName(propertyName);
                                    jsonWriter.writeOpenBracket();
                                    Object[] o = (Object[])obj;
                                    for (int i = 0; i < o.length; ++i) {
                                        saveObjectAsJSON(o[i]);
        ...
        if (readMethod != null) {
            if (JSONUtil.primitiveArraysSet.contains(propertyType)) {
                if (propertyType == int[].class) jsonWriter.writeArrayOfPrimitives((int[]) readMethod.invoke(invoker));
                    ...
                    jsonWriter.writeArrayOfWrappers((Object[]) readMethod.invoke(invoker));
            } else {
                try {
