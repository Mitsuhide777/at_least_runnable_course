...
//-------------- E.G.-------------------------
//	    return BeanBoxFrame.getAbsTmpDir()+File.separator+"___comp_ROOT.ser";
//--------------------------------------------
		}
		else 
		{
...
//-------------- E.G.-------------------------
//		s = BeanBoxFrame.getAbsTmpDir()+File.separator+name;
//--------------------------------------------
				serNames.put(o, s);
			}
			return s;
...
System.err.println(getClass().getName()+".save() invoked.");
...
//-----------------------------------------
    ...
    private void serializeComponent() 
	{
		...
		Wrapper wrap = BeanBoxFrame.getCurrentWrapper();

		for (int j = 0; j < getComponentCount(); ++j) {
			if (((Wrapper)getComponent(j)).getChild().getName() ==null) {
				((Wrapper)getComponent(j)).getChild().setName("abc" + j);
			}
		}
		...
		try
		{
			...
			JSONDecoder jd = new JSONDecoder(everything);
			Object des = jd.readJSON();
			System.out.println("");
			Sample s = (Sample)des;

			Object[] arr = s.getArr();
			Map<String, Vector> events = jd.getEvents();
			for (Map.Entry<String, Vector> entry : events.entrySet()) {
				Wrapper source = null;
				for (int i = 0; i < arr.length; ++i) {
					Wrapper current = (Wrapper)arr[i];
					if (entry.getKey().equals(current.getChild().getName())) {
						source = current;
						break;
					}
				}
				...
				for (int j = 0; j < entry.getValue().size(); ++j) {
					WrapperEventInfo eventInfo = (WrapperEventInfo)(entry.getValue().get(j));

					for (int m = 0; m < arr.length; ++m) {
						Wrapper target = (Wrapper) arr[m];
						if (target.getChild().getName().equals(((Component) eventInfo.getTargetBean()).getName())) {
							Method add = Wrapper.class.getDeclaredMethod("addEventTarget", String.class, Wrapper.class, Object.class);
							add.setAccessible(true);

							Class<?> hookUp = Class.forName(eventInfo.getAdaptorClassName());
							...
							set.invoke(listener, target.getBean());

							add.invoke(source, eventInfo.getEventSetName(), target, listener);
						}
					}
				}
			}
			...
			{
				Component c = (Component)s.getArr(i);
				Wrapper w = (Wrapper) c;
...
//			add((Component)des);
//			((Component)des).invalidate();
/*			JSONEncoder js = new JSONEncoder();
			Sample sample = new Sample();
			sample.setArr(new Object[getComponentCount()]);
			for (int j = 0; j < getComponentCount(); ++j) {
				sample.setArr(j, (Wrapper)getComponent(j));
			}
			js.saveJSON(sample);
			PrintWriter pw = new PrintWriter(file);
			pw.write(js.JSONasString());
			pw.close();*/
...
//-------------------- E.G.--------------------
//SimpleClassLoader.ourLoader.printRawClasses();
//SimpleClassLoader.ourLoader.printResourceHash();
//SimpleClassLoader.ourLoader.printMimeHash();
//---------------------------------------------
		removeAll();
...
//*/
			       ...
			       java.io.IOException 
	{
		...
		ClassLoader cl = SimpleClassLoader.ourLoader;
//---------------- E.G.----------------
//((SimpleClassLoader)cl).printRawClasses();
//-------------------------------------
    ...
    void doBind() 
	{
		...
		PropertyNameDialog dialog = new PropertyNameDialog
										(
											getFrame(),
											bean,
    ...
    public void mouseReleased(MouseEvent evt) 
	{
		...
		if (deltaWhen < 5000) 
		{
			...
			synchronized (this) 
			{
	        	mouseClickEvent = new MouseEvent(
										evt.getComponent(), 
										evt.getID(),
										evt.getWhen(),
										evt.getModifiers(),
    ...
    void doMenuItem(ActionEvent evt) 
	{
		...
		else if (parent.getLabel().equals("Edit")) 
		{
			if (label.equals("Cut")) 
			{
				//try //E.G. for debug only
				//{
					cut();
				//}
				//catch(Exception e)
				//{
				//	System.err.println("Cut: Exception = "+e);
				//}
			}
		...
		else if (parent.getLabel().equals("InfoBus")) 
		{
    ...
    Rectangle getResizeBox(int mx, int my) 
	{
		...
		if (resizeCursor == swResizeCursor) 
		{
			w = w - mx;
			h = my;
			x = x + mx;
		...
		if (resizeCursor == neResizeCursor) 
		{
			w = mx;
			h = h - my;
			y = y + my;
		...
		if (resizeCursor == seResizeCursor) 
		{
			w = mx;
			h = my;
		}
